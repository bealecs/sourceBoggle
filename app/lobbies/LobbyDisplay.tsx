...
export const LobbyDisplay = ({ params }: { params: Lobby[] }) => {
  ...
  return (
    ...
    <ul className="flex-col w-full text-xl mt-24 px-2">
      ...
      {params.map((lobby, index) => (
        <div key={index} className="flex content-center items-center justify-evenly my-8 border-2">
          <li className="border-x-2 p-4 w-3/12">
            Game status: <span className="text-blue-500">{lobby.game_active ? "🟢 Active" : "🔴 Waiting in lobby"}</span>
          </li>
          <li className="border-x-2 p-4 w-3/12">Lobby Code: <span className="text-blue-500">{lobby.game_code}</span></li>
          <li className="border-x-2 p-4 w-3/12">Player count: <span className="text-blue-500">{lobby.players.length}</span></li>
